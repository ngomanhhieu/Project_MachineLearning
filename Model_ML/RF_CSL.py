...
def bitmap_encode(df):
    ...
    for col in df.columns:
        if col in bitmap_rules:
            ...
            encoded_df[col] = df[col].apply(lambda x: 1 if x > median else 0)
    return encoded_df
...
def main():
    ...
    model_performance = pd.DataFrame(columns=[
        "Accuracy", "Recall", "Precision", "F1-Score", "Train Time", "Predict Time", "Total Time"
    ])

    # CSL + Random Forest
    rf_model = RandomForestClassifier(n_estimators=100, random_state=42)
    evaluate_model("CSL Random Forest", rf_model, X_train_bitmap, X_test_bitmap, y_train, y_test, model_performance)
