...
def bitmap_encode(df):
    ...
    for col in df.columns:
        if col in bitmap_rules:
            ...
            encoded_df[col] = df[col].apply(lambda x: 0)
    return encoded_df
...
def main():
    ...
    model_performance = pd.DataFrame(
        columns=["Accuracy", "Recall", "Precision", "F1-Score", "Train Time", "Predict Time", "Total Time"]
    )

    # CSL using Logistic Regression
